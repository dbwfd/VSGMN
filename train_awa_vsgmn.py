...
wandb.init(project='VSGMN', config='wandb_config/awa2_czsl_best.yaml')
config = wandb.config
...
for i in range(0, niters):
    ...
    loss, loss_CE, loss_cal, loss_reg, loss_kl = out_package['loss'], out_package[
        'loss_CE'], out_package['loss_cal'], out_package['loss_reg'], out_package['loss_kl']
    ...
    if i % report_interval == 0:
        ...
        print('iter/epoch=%d/%d | loss=%.3f, loss_CE=%.3f, loss_cal=%.3f, '
              'loss_reg=%.3f, loss_kl=%.3f| acc_unseen=%.3f, acc_seen=%.3f, H=%.3f | '
              'acc_zs=%.3f' % (
                  i, int(i // report_interval),
                  loss.item(), loss_CE.item(), loss_cal.item(),
                  loss_reg.item(), loss_kl.item(),
