...
class CUBDataLoader():
    ...
    def get_vp(self):
        mask = torch.eye(200).to(self.device)
        mask[:,self.unseenclasses]=1
        mask=mask==1
        att_sim=torch.matmul(self.att,self.att.T)
        att_sim[mask]=0
        unseen_sim=att_sim[self.unseenclasses]

        a, index1 = torch.sort(unseen_sim, dim=-1)
        a[:, 0:200 - 10] = 0
        b, index2 = torch.sort(index1)
        unseen_sim = torch.gather(a, dim=1, index=index2) #10最近邻
        unseen_sim[unseen_sim!=0]=0.1



        self.vp=torch.zeros(200,2048,14,14).to(self.device)
        seenvp=[]
        mylist=self.idxs_list
        myfeature=self.data['train_seen']['resnet_features']

        for item in mylist:
            classfeature=myfeature[item]
            classvp=classfeature.mean(dim=0)
            seenvp.append(classvp)
        for index,item in enumerate(seenvp):
            allindex=self.seenclasses[index]
            self.vp[allindex]=item


        unseenvp=torch.einsum('ij,jkmn->ikmn',[unseen_sim,self.vp])

        for index,item in enumerate(unseenvp):
            allindex=self.unseenclasses[index]
            self.vp[allindex]=item
...
class SUNDataLoader():
    ...
    def get_idx_classes(self):
        ...
        return self.idxs_list
    def get_vp(self):
        mask = torch.eye(717).to(self.device)
        mask[:,self.unseenclasses]=1
        mask=mask==1
        att_sim=torch.matmul(self.att,self.att.T)
        att_sim[mask]=0
        ...
        self.vp=torch.zeros(717,2048,14,14).to(self.device)
        seenvp=[]
        mylist=self.idxs_list
        myfeature=self.data['train_seen']['resnet_features']
...
class AWA2DataLoader():
    def __init__(self, data_path, device, is_scale=False,
                 is_unsupervised_attr=False, is_balance=True,is_episode=False,n_gclass=0):
        print(data_path)
        sys.path.append(data_path)
        self.data_path = data_path
        self.device = device
        ...
        if self.is_episode:
            print('episode dataloader')
            self.n_gclass = n_gclass
    ...
    def next_batch(self, batch_size):
        if self.is_balance:
            ...
            idx = []
            n_samples_class = max(batch_size // self.n_gclass, 1)
            sampled_idx_c = np.random.choice(np.arange(self.ntrain_class), min(self.n_gclass, batch_size),
        ...
        batch_att = self.att[batch_label].to(self.device)
        if self.is_episode:
            batch_class=torch.unique(batch_label)
            return batch_label, batch_feature, batch_att,batch_class
        else:
            return batch_label, batch_feature, batch_att
    ...
    def get_idx_classes(self):
        ...
        return self.idxs_list
    def get_vp(self):
        mask = torch.eye(50).to(self.device)
        mask[:,self.unseenclasses]=1
        mask=mask==1
        att_sim=torch.matmul(self.att,self.att.T)
        att_sim[mask]=0
        ...
        self.vp=torch.zeros(50,2048,14,14).to(self.device)
        seenvp=[]
        mylist=self.idxs_list
        myfeature=self.data['train_seen']['resnet_features']
